...
class LightsView extends Component {
    ...
    fetchData() {
        let url = this.getUrlWithUsername();                    // Asking getUrlWithUsername for the API URL

        fetch(url)                                              // Init fetch with url
            .then(response => {                                     // Take API response -> 
    ...
    render() {
        ...
        let brighnessHandler = this.onBrightnessChanged;
        Object.keys(data).forEach(function(id,index) {
            ...
            let light = <LightItem  key={id} id={id} name={data[id].name}
                                    ...
                                    onBrightnessChanged={brighnessHandler}/>
